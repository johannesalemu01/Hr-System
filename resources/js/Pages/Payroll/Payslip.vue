<template>
    <div class="max-w-4xl mx-auto" ref="payslipContent">
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-gray-600">
                        For the period:
                        {{ formatDate(payrollItem.payroll_period.start_date) }}
                        - {{ formatDate(payrollItem.payroll_period.end_date) }}
                    </p>
                </div>
            ...
            <div
                class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 border-b border-gray-200 pb-6"
            >
                <div>
                    <h3 class="text-lg font-semibold mb-2">
                        Company Information
                    </h3>
                    <p class="text-gray-700">Omishtu-joy</p>
                    <p class="text-gray-600">megenagna metebaber bldg.</p>
                    <p class="text-gray-600">Addis Ababa</p>
                    <p class="text-gray-600">Phone: (123) 456-7890</p>
                    <p class="text-gray-600">Email: omishtu@gmail.com</p>
                </div>
                <div>
                    <h3 class="text-lg font-semibold mb-2">
                    ...
                    <p class="text-gray-600">
                        Employee ID: {{ payrollItem.employee.employee_id }}
                    </p>
                    <p class="text-gray-600">
                        Department: {{ payrollItem.employee.department }}
                    </p>
                    <p class="text-gray-600">
                        Position: {{ payrollItem.employee.position }}
                    </p>
                    <p class="text-gray-600">
                        Join Date:
                        {{ formatDate(payrollItem.employee.join_date) }}
                    </p>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <p class="text-gray-700">
                            {{
                                formatDate(
                                    payrollItem.payroll_period.payment_date
                                )
                            }}
                        </p>
                        ...
                        <p class="text-gray-700">
                            {{ payrollItem.employee.bank_name }}
                        </p>

                        <p class="text-gray-600">Account Number:</p>
                        <p class="text-gray-700">
                            {{ payrollItem.employee.bank_account }}
                        </p>
                    </div>
                </div>
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <p class="text-gray-700">
                            {{ payrollItem.working_days }} days
                        </p>

                        <p class="text-gray-600">Leave Days:</p>
                        <p class="text-gray-700">
                            {{ 22 - payrollItem.working_days }} days
                        </p>
                        ...
                        <tbody>
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="py-2 text-right text-gray-700">
                                    {{
                                        formatCurrency(
                                            payrollItem.earnings.basic_salary
                                        )
                                    }}
                                </td>
                            </tr>
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="py-2 text-right text-gray-700">
                                    {{
                                        formatCurrency(
                                            payrollItem.earnings.allowances
                                        )
                                    }}
                                </td>
                            </tr>
                            <tr
                                v-for="bonus in payrollItem.earnings.bonuses"
                                :key="bonus.type"
                            ...
                            <tr class="font-semibold">
                                ...
                                <td class="py-2 text-right text-green-600">
                                    {{
                                        formatCurrency(
                                            payrollItem.earnings.total_earnings
                                        )
                                    }}
                                </td>
                            </tr>
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                <p class="text-gray-600 mt-2">
                    {{ amountInWords(payrollItem.net_pay) }}
                </p>
            </div>
        </div>
...
const amountInWords = (amount) => {
    ...
    if (birrs > 0) {
        if (birrs < 1000) {
            ...
            result =
                convertLessThanOneThousand(Math.floor(birrs / 1000)) +
                " Thousand";
            if (birrs % 1000 !== 0) {
                result += " " + convertLessThanOneThousand(birrs % 1000);
            } else {
                result =
                    convertLessThanOneThousand(Math.floor(birrs / 100000)) +
                    " Lakh";
                if (birrs % 100000 !== 0) {
                    if (birrs % 100000 < 1000) {
                        result +=
                            " and " +
                            convertLessThanOneThousand(birrs % 100000);
                    } else {
                        result +=
                            " " +
                            convertLessThanOneThousand(
                                Math.floor((birrs % 100000) / 1000)
                            ) +
                            " Thousand";
                        if (birrs % 1000 !== 0) {
                            result +=
                                " " + convertLessThanOneThousand(birrs % 1000);
                        }
                    }
                }
            }
        ...
        if (paise > 0) {
            result +=
                (birrs > 0 ? " and " : "") +
                convertLessThanOneThousand(paise) +
                " Paise";
        }
