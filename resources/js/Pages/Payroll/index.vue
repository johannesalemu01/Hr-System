<template>
  <AuthenticatedLayout title="Payroll" description="You can show and manage Payroll from here.">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
      <div class="flex items-center">
        ...
        <div>
          ...
          <p class="text-gray-600">You can show and manage Payroll from here.</p>
        </div>x
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div class="flex items-center">
          <button class="bg-gray-800 text-white px-4 py-2 rounded-md flex items-center">
              ...
              <div class="relative">
                <ListboxButton class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                  <div class="flex items-center">
                    ...
                    <span class="block truncate">{{ currentPeriod.formatted }}</span>
                  </div>
                ...
                <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                  <ListboxOptions class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                    <ListboxOption v-for="period in payrollPeriods" :key="period.id" v-slot="{ active, selected }" :value="period">
                      <div :class="[active ? 'text-white bg-primary-600' : 'text-gray-900', 'cursor-default select-none relative py-2 pl-3 pr-9']">
                        <span :class="[selected ? 'font-semibold' : 'font-normal', 'block truncate']">{{ period.label }}</span>
        ...
        <!-- Payslip Button -->
        <button 
          class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md flex items-center justify-center"
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="item in payrollItems.data" :key="item.id">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10">
                    <img class="h-10 w-10 rounded-full" :src="item.employee.profile_picture" alt="" />
                  </div>
                  <div class="ml-4">
                    <div class="text-sm font-medium text-gray-900">{{ item.employee.name }}</div>
                    <div class="text-sm text-gray-500">{{ item.employee.employee_id }}</div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium" :class="item.net_pay < 0 ? 'text-red-600' : 'text-green-600'">
      ...
      <div v-if="payrollItems.data.length > 0" class="mt-6">
        <Pagination :links="payrollItems.links" :meta="payrollItems.meta" />
      </div>
    </div>
...
const props = defineProps({
  payrollItems: {
    type: Object,
    required: true, // Ensure this includes pagination data
  },
  payrollPeriods: {
    type: Array,
    required: true
...
const generateAllPayslips = () => {
  // This would typically download a ZIP of all payslips
  // For now, we'll just show an alert
  alert('This would download all payslips for the current period.');
};

// Process payroll
const processPayroll = () => {
  if (confirm('Are you sure you want to process this payroll? This will lock the payroll for editing.')) {
...
const releasePayroll = () => {
  if (confirm('Are you sure you want to release this payroll? This will mark the payroll as paid.')) {
